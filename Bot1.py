...
import sys
import csv
#import CrealienSizeOptimization
...
pygame.display.set_caption('Bot 1 Part 2  Algo')
...
#crew_alien_grid = CrealienSizeOptimization.getAlienCrewsize()
alien_threshold = 0
grid_distances = {}
...
f=open('proj2AlienCrewDetectionSquareAnalysisLatest.csv','a')
writer=csv.writer(f)
ans = 0
idleTime=0
while simulate:
    ...
    while initial_node != desired_node:
        ...
        if initial_node in aliens:
            ...
            writer.writerow([no_of_moves,idleTime,"alien Found"])
            f.close()
            sys.exit(1)
        ...
        print("max_prob = ", max_prob, "max_nodes = ", max_nodes)
        move_to = random.choice(max_nodes)
        print("The move to value is ", move_to)
        path = astar(grid, initial_node, desired_node, aliens)
        initial_node = path[1]
    ...
    if  initial_node == desired_node  :
        writer.writerow([no_of_moves,idleTime,"crew Found"])
        f.close()
        sys.exit(1)
