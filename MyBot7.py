...
#oper can be "outsideBorder" "presentInside" "outsideUpdate" "stepOneInside" "stepOneInside"
def update_crew_alien_grid_AlienMoved(grid,crew_alien_grid,x,y,prev, crew_found_inside_detection_grid, blocked_nodes, movement ):
            ...
            if oper == "stepOneInside":
                ...
                if (i,j) in blocked_nodes:
                    ...
                    for possible_move in possible_moves:
                        crew_alien_grid[possible_move[0]][possible_move[1]]+=copy_crew_alien_grid[i][j]/len(possible_moves)
    ...
    if oper == "outsideBorder":
        ...
        if movement == (1,0):
            if x_start-1>=0:
                for j in range(y_start,y_end+1):
                    if j>=0 and j<=size-1:
                        possible_moves=getPossibleMoves(x_start-1,j,blocked_nodes+opened_nodes_inside_crew_alien)
                        summation+=copy_crew_alien_grid[x_start-1][j]
                        for poss_move in possible_moves:
                            crew_alien_grid[poss_move[0]][poss_move[1]]+=(copy_crew_alien_grid[x_start-1][j]/len(possible_moves))
            ...
            if x_end+1<=size-1:
                for j in range(y_start,y_end+1):
                    if j>=0 and j<=size-1:
                        ...
                        for poss_move in possible_moves:
                            crew_alien_grid[poss_move[0]][poss_move[1]]+=(copy_crew_alien_grid[x_end+1][j]/len(possible_moves))
        ...
        if movement == (0,1):
            if y_start-1>=0:
                for i in range(x_start,x_end+1):
                    if i>=0 and i<=size-1:
                        possible_moves=getPossibleMoves(i,y_start-1,blocked_nodes+opened_nodes_inside_crew_alien)
                        summation+=copy_crew_alien_grid[i][y_start-1]
                        for poss_move in possible_moves:
                            crew_alien_grid[poss_move[0]][poss_move[1]]+=(copy_crew_alien_grid[i][y_start-1]/len(possible_moves))
            ...
            if y_end+1<=size-1:
                for i in range(x_start,x_end+1):
                    if i>=0 and i<=size-1:
                        ...
                        for poss_move in possible_moves:
                            crew_alien_grid[poss_move[0]][poss_move[1]]+=(copy_crew_alien_grid[i][y_end+1]/len(possible_moves))
            ...
            for j in range(size):
                if (i,j) not in blocked_nodes:
                    ...
                    for possible_movement in possible_movements:
                        ...
                        crew_alien_grid[ possible_movement[0] ][possible_movement[1]]+= copy_crew_alien_grid[i][j]/len(possible_movements)     
                 


            
    if oper=="stepOneOutside" or oper=="stepOneInside":
        for i in range(size):
            for j in range(size):
            ...
            for j in range(size):
                if (i,j) not in blocked_nodes:
                    ...
                    for move in move_set:
                        ...
                        if (adj_i,adj_j) not in blocked_nodes :
                            possible_movements.append((adj_i,adj_j))
...
def generate_alien_pairs(grid,blocked_nodes):
    # alien_pairs=np.full((size,size,size,size), 0.0)
    # total=0
    # for i in range(size):
    #     for j in range(size):
    #         for k in range(size):
    #             for l in range(size):
    #                 if i!=k and j!= l  and (i,j) not in blocked_nodes and (k,l) not in blocked_nodes and alien_pairs[k,l,i,j] == 0.0:
    #                     alien_pairs[i,j,k,l]=1.0
    #                     total+=1
                        
    # return alien_pairs/total
    total=0
    part1=[]
    part2=[]
    part3=[]
        ...
        for j in range(size):
            ...
            for k in range(size):
                for l in range(size):
                    ...
                    for p in range(len(part2)):
                        if part2[p]==(i,j) and part1[p]==(k,l):
                            flag=1
                    
                    
                    if i!=k and j!= l  and (i,j) not in blocked_nodes and (k,l) not in blocked_nodes and flag==0:
                        part1.append((i,j))
                        part2.append((k,l))
                        part3.append(0)
    ...
    return [part1,part2,part3]                  
                    #Need to check if if the opposite way is alredy preasent at a later point
                    
def update_alien_pairs(x,y,blocked_nodes,alien_pairs,prev,crew_found_inside_detection_grid):
    ...
    if oper=="inside":
        ...
        for i in range(len(part0)):
            for j in range(i,len(part1)):
                temp=0
                if check_if_neighbour(part0[i], part0[j]) and check_if_neighbour( part1[i], part1[j])  :
                    temp+=((1/len(getPossibleMoves( part0[j][0] ,part0[j][1] , blocked_nodes))) *  (1/len(getPossibleMoves( part1[j][0] ,part1[j][1] , blocked_nodes)) * part2[j] ))
                    
                    
                
                if check_if_neighbour(part0[i], part1[j]) and check_if_neighbour(part1[i], part0[j]):
                    temp+=((1/len(getPossibleMoves( part1[j][0] ,part1[j][1] , blocked_nodes))) *  (1/len(getPossibleMoves( part0[j][0] ,part0[j][1] , blocked_nodes)) * part2[j] ))
...
def check_if_neighbour(nodea,nodeb):
    move_set = ((0, -1), (0, 1), (-1, 0), (1, 0))
    
    for move in move_set:
        p_adj = nodea[0] + move[0]
        q_adj = nodea[1] + move[1]
        if p_adj == nodeb[0] and q_adj==nodeb[1]:
            return True
...
alien_pairs=generate_alien_pairs(grid, blocked_nodes)
...
while simulate:
    ...
    while len(desired_nodes) > 0:
        ...
        alien_pairs=update_alien_pairs(initial_node[0],initial_node[1], blocked_nodes, alien_pairs, prev, crew_found_inside_detection_grid)
